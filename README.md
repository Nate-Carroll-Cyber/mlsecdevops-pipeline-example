...
- **Split Prompt Roles**: System configuration keeps the editable **Safeguard Effective Prompt** distinct from the **Downstream Responder Prompt**. The safeguard path uses one canonical effective prompt containing the runtime JSON verdict contract, neutral evidence contract, category guidance, and promoted few-shot examples. The downstream responder prompt is sent only after the safeguard judge returns `CLEAN` and responder routing is enabled.
- **Safeguard Effective Prompt Review**: System Configuration displays and edits the exact effective safeguard prompt sent as the decision-model instruction. Recommended and current SHA-256 hashes are shown side by side so intentional prompt updates can be promoted as the new baseline.
...
The Docker demo also starts `counter-spy-postgres` on host port `15432` for the instruction monitor. The backend initializes the pgvector schema lazily and can generate embeddings server-side when a dedicated local/OpenAI-compatible embeddings provider is configured with `INSTRUCTION_MONITOR_EMBEDDINGS_*`, or when the local LM Studio safeguard base URL is available. Embeddings do not inherit the generic responder or OpenAI LLM endpoint. API callers may still supply `metadata.instructionEmbedding` or `metadata.instructionChunks`, but normal frontend submissions no longer require the browser to calculate embeddings. The demo Postgres data directory is tmpfs-backed, so recreating the container gives you a fresh instruction database by design.
