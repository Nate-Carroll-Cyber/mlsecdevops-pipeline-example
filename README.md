...
- **Safeguard Runtime Verdict Contract**: Current safeguard enforcement uses one runtime JSON contract only: `{"verdict":"CLEAN|SUSPICIOUS|ADVERSARIAL","analystReasoning":"brief reason"}`. Legacy decision-shaped payloads such as `ALLOW_AND_FORWARD`, malformed JSON, or non-JSON safeguard responses fail secure to `SUSPICIOUS` / `QUEUED` and are never treated as allow-by-default.
