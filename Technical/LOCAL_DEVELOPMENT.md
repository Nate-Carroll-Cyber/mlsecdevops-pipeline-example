...
The Docker demo reads local-only database secrets from `.env.demo.local`. Set `POSTGRES_PASSWORD` there and pass the file to Compose with `--env-file .env.demo.local` so the Postgres container receives only the database password it needs. The gateway derives the demo connection string from `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_DB`, `POSTGRES_USER`, and `POSTGRES_PASSWORD`; set `INSTRUCTION_MONITOR_DATABASE_URL` only when overriding the default demo database target. The compose stack does not hard-code the password, binds Postgres only to localhost, initializes the database with SCRAM authentication and data checksums, and applies defensive runtime settings for connection logging, DDL logging, slow-query logging, statement timeout, and max connections.
