...
Flow (mirrors the Community API's batched search usage):
  ...
  4. Gate: fail when any pin trips a category named in --fail-on (malware,
     tampering). A "warning" verdict (the middle of pass/warning/fail) is
     surfaced in the report + summary but does NOT block unless its category is
     named in --warn-as-fail (RL_WARN_AS_FAIL). With --fail-on empty the job is
...
def check_account(base: str, token: str, timeout: int) -> int:
    ...
    try:
        ...
        print(f"TOKEN CHECK SKIPPED — could not reach {url}: {exc}")
        print("  (infra/network error, not a token failure — proceeding to the scan.)")
        return 0
    ...
    if resp.status_code != 200:
        # A 404 (or other non-auth status) means this account/health endpoint isn't
        # available on the configured API host/tier — NOT that the token is bad. Don't
        # red the pipeline on a preflight endpoint mismatch: warn and proceed to the
        # scan, where a genuinely bad token surfaces as an auth error. Only 401/403
        # (handled above) fail the preflight.
        print(f"TOKEN CHECK SKIPPED — account endpoint returned HTTP {resp.status_code} "
              f"(not available on this API host/tier): {resp.text[:120]}")
        return 0
...
def evaluate_package(purl: str, pkg: dict, want_version: str | None,
                     fail_on: set[str], warn_as_fail: set[str]) -> dict:
    """Build a per-package result and decide which categories it trips.

    `hits` drive the gate (blocking). `warns` record any "warning" verdict — the
    middle of the pass/warning/fail scale — which is surfaced but only blocks when
    its category is named in warn_as_fail (the RL_WARN_AS_FAIL switch).
    """
    ...
    # A "warning" verdict is recorded for every known category (honest evidence) and,
    # if that category is named in warn_as_fail, promoted to a blocking hit. Default
    # (empty warn_as_fail) leaves warnings non-blocking — they still appear in the
    # report and summary, so the gate result no longer hides them.
    warns: list[dict] = []
    for cat in sorted(KNOWN_CATEGORIES):
        if statuses.get(cat) == "warning":
            warns.append({"category": cat, "reason": f"assessments.{cat}.status=warning"})
            if cat in warn_as_fail:
                hits.append({"category": cat,
                             "reason": f"assessments.{cat}.status=warning (RL_WARN_AS_FAIL)"})
...
def main() -> None:
    ...
    parser.add_argument("--warn-as-fail", default="",
                        help="comma list of categories whose 'warning' verdict is "
                             "promoted to a blocking hit (malware,tampering). Blank "
                             "(default) = warnings are surfaced in the report/summary "
                             "but never block.")
    ...
    if unknown:
        ...
        raise SystemExit(2)
    warn_as_fail = {c.strip().lower() for c in args.warn_as_fail.split(",") if c.strip()}
    unknown_warn = warn_as_fail - KNOWN_CATEGORIES
    if unknown_warn:
        print(f"ERROR: unknown --warn-as-fail categor(y/ies): {', '.join(sorted(unknown_warn))} "
              f"(known: {', '.join(sorted(KNOWN_CATEGORIES))})")
        write({"skipped": True, "reason": f"unknown warn-as-fail categories: {sorted(unknown_warn)}"})
        raise SystemExit(2)
    # warn_as_fail only has teeth in enforce mode, so a warn-as-fail category also
    # turns the gate on (otherwise it'd compute hits then return report-only).
    enforce = bool(fail_on or warn_as_fail)
    ...
    print(f"  scanning {len(items)} unique {args.ecosystem} package(s) from "
          ...
          f"{'; warn-as-fail ' + ','.join(sorted(warn_as_fail)) if warn_as_fail else ''}")
    ...
    for batch in chunked(items, batch_size):
        ...
        for entry in community.get("packages") or []:
            ...
            results.append(evaluate_package(uuid, entry.get("package") or {},
                                            want_version, fail_on, warn_as_fail))
    ...
    warnings = [
        {"purl": r["purl"], **warn}
        for r in results for warn in r["warns"]
    ]
    ...
    for r in results:
        ...
        # Surface non-blocking warnings too (blocking ones already show as HIT above).
        non_block = sorted({w["category"] for w in r["warns"] if w["category"] not in warn_as_fail})
        if non_block:
            print(f"    WARN [{', '.join(non_block)}] {r['purl']} "
                  f"(version {r['assessed_version']}, statuses={r['statuses']}) — non-blocking")
    ...
    if warnings:
        print(f"Spectra Assure Community gate PASSED — no BLOCKING malware/tampering verdicts; "
              f"{len(warnings)} non-blocking warning(s) surfaced (see report 'warnings'; "
              f"set RL_WARN_AS_FAIL to block on them)")
    else:
