...
- **Safeguard Credential Separation**: Operators can supply browser-local safeguard Base URL, Model ID, and memory-only API key overrides per browser for demos, while backend `SAFEGUARDS_*` runtime configuration or a secret manager remains the preferred operational credential path.
...
- **Downstream Responder Runtime View**: The dedicated Responder tab surfaces provider, backend health, model/base URL, responder key source, context-window configuration, last forwarded prompt hash, responder latency, token usage, and output-sanitization flags without exposing guardrail toggles on the responder surface.
...
- **Prompt Role Separation**: System configuration now distinguishes a firewall enforcement prompt from a downstream responder prompt. The firewall prompt plus guardrails policy govern the safeguard judge's inspection and forwarding decision, while the backend-owned JSON verdict contract remains outside the user-visible prompt. The downstream responder prompt is sent to the responder model only after clean traffic clears the gateway.
...
- **Sam Spade Response Model Boundary**: Clean Sam Spade turns now use the live downstream responder path after local and safeguard approval. The responder receives the active Downstream Responder Prompt plus admin-managed Sam Spade persona and scenario prompts, and resulting review artifacts may include `sam_spade_ctf` prompt-profile telemetry, responder provider, responder model, status, and latency.
