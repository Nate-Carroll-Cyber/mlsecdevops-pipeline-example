...
- **Forbidden Phrases Governance**: Configurable normalized phrase-family matching for forbidden phrases, combined with explicit forbidden-category guidance inside the saved firewall prompt so the active decision model can treat indirect, paraphrased, translated, or obfuscated forms as non-allow categories.
...
- **Backend-Owned Responder Secrets**: Browser-side API key injection remains removed. Operators can now supply browser-local responder Base URL and Model ID overrides per browser, but credentials still live in backend runtime configuration or a secret manager.
...
- **Prompt Role Separation**: System configuration now distinguishes a firewall enforcement prompt from a downstream responder prompt. In the current runtime, the visible firewall prompt plus the active guardrails policy and relevant Knowledge Base context guide the active decision model, while the downstream responder prompt remains reserved for a later multi-stage architecture.
...
- **Saved Firewall Category Guidance**: The baseline firewall prompt now explicitly lists forbidden request categories such as financial advice, political persuasion, religious guidance, medical advice, legal advice, mental health crisis counseling, self-harm assistance, violence and weapon construction, malware or exploit development, credential theft, secrets exfiltration, fraud/social engineering, privacy invasion, doxxing, illicit drug production, extremist or hate content, sexual content involving minors, and obvious obfuscation / gibberish / made-up-language concealment. Storytelling remains explicitly non-exempt when used to smuggle one of those categories.
