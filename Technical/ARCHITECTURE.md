...
*   **CTF frontend container** — the noir UI (`ctf-frontend/`, image `Dockerfile.ctf-frontend`) also runs on its own. It talks to the gateway, plays the game, and `POST`s each turn's review artifact to `POST /v1/ctf/review-artifacts` (an in-memory ring buffer on the gateway; `GET /v1/ctf/review-artifacts?sinceTimestamp=&limit=` reads it back). The main Counter-Spy frontend, when `VITE_CTF_FRONTEND_URL` is set, embeds the CTF app in the Sam Spade tab via an `<iframe>` and polls that feed to mirror CTF turns into Analyst Chat + `audit_logs` (source `ctf_chat`) — so Audit/Metrics keep surfacing CTF activity even though the CTF UI no longer drives the CTF API. With `VITE_CTF_FRONTEND_URL` unset, the main frontend renders the in-app CTF UI and drives the routes directly as before.
...
*   The HTTP contract (see `Technical/SAM_SPADE_API_CONTRACT.md`) is unchanged across the split. The review-artifact buffer is intentionally in-memory and lossy — a demo bridge, not durable storage; a UI that wasn't open misses turns that scrolled out of the 500-entry buffer.
