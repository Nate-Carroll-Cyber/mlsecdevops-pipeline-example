...
- **#8 — Frontend logging (done).** `src/lib/devLog.ts` exports `devLog` / `devWarn` that are no-ops in a production build (`import.meta.env.PROD`); the five debug `console.log`/`console.warn` calls in `src/App.tsx` now use them. `src/lib/firebase.ts` `safeParse`s `firebase-applet-config.json` and throws a generic error naming only the bad keys (never echoing config values). Genuine `console.error` calls are left as-is — errors should surface in any environment.
