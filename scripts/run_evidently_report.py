...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Write --seed-out from the CURRENT dataset even when a reference already "
            "exists. This is how the committed drift reference is intentionally "
            "REFRESHED (triggered by [refresh-drift-reference] in the commit message). "
            "Without this flag a seed is written only on first activation, i.e. when no "
    ...
    def write_seed(df) -> Path:
        seed = Path(args.seed_out)
        seed.parent.mkdir(parents=True, exist_ok=True)
        seed.write_text(
            "\n".join(json.dumps(_clean(r)) for r in df.to_dict(orient="records")) + "\n",
            encoding="utf-8",
        )
        return seed
    ...
    if not reference_path.exists():
        # First activation: no reference committed yet — seed one for the maintainer.
        seed = write_seed(current_df)
    ...
    # A reference exists. Normally we only compare. With --force-seed we ALSO re-seed
    # from the current dataset, so data-drift-baseline-commit can replace the committed
    # reference (an intentional refresh). The comparison below still runs against the
    # OLD reference, so the run also shows how far the new data drifted before adoption.
    if args.force_seed:
        seed = write_seed(current_df)
        print(f"[refresh] --force-seed set — re-seeded reference from current dataset "
              f"({len(current_df)} record(s)) → {seed}")
        print("  data-drift-baseline-commit will REPLACE the committed reference with it.")
