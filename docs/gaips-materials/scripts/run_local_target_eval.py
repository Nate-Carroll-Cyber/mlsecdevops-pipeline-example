...
def main() -> None:
    ...
    report = {"tool": "gaips-local-target-eval", "mode": "live-local", "results": results, "passed": all(r["pass"] for r in results)}
    (reports / "local-target-results.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
