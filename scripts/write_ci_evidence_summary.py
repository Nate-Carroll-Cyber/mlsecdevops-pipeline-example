...
def verdict(name: str, path: Path) -> tuple[str, str]:
    ...
    *failing* evidence from a *complete* one — the old gate only saw existence."""
    ...
    if name == "dvc-status.json":
        ...
        # un-evaluated run (pull/status failed) → inert.
        if doc.get("evaluated") is False:
            return "inert", doc.get("reason", "not evaluated")
        if "in_sync" in doc:
            return ("pass", "workspace matches pinned versions") if doc.get("in_sync") \
                else ("fail", f"{doc.get('drift_count', 'some')} tracked artifact(s) drifted from pinned version")
        return "inert", "present (no verdict)"
    # ydata-profile.json — descriptive, no verdict.
    return "inert", "present (no verdict)"
