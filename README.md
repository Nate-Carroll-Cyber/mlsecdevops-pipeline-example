...
- **Entropy Analysis (Sliding Window)**: Calculates Shannon entropy using a sliding window approach (35-character windows, 5-character steps) to detect localized high-entropy payloads (Base64, Hex, etc.) and prevent **Token Dilution** attacks.
  ...
  - **Thresholds**: Entropy now follows one explicit governance policy across Analyst Chat, Playground, Audit Logs, and Metrics. Prompt entropy at or below `3.2` is treated as allowed on entropy grounds alone. Entropy above `3.2` and up to the configured `Entropy Threshold` is treated as `Suspicious`. Entropy above the configured threshold is treated as `Adversarial`.
...
- **Keyword & Topic Filtering**: Matches input against configurable lists of blocked phrases and forbidden topics. Today, `Blocked Topics` remains a normalized phrase-matching control rather than a standalone semantic classifier; semantic category steering is currently reinforced through the saved firewall prompt. The default blocklist also includes risky transport markers such as `javascript:` and `://`, and redacts policy-triggering script/URL payloads before they are persisted in audit detail views.
...
- **Suspicious Interception**: If entropy rises above `3.2` without exceeding the configured adversarial cutoff, or if blocked keywords, forbidden topics, suspicious external-call attempts, or other supporting structural signals are detected, the request is blocked pre-inference as `Suspicious`.
- **Human-in-the-Loop (HITL) Mode**: When activated, automatically intercepts borderline traffic using the live governance thresholds (default entropy threshold `4.0` as the adversarial cutoff, a fixed suspicious entropy floor at `3.2`, plus the active syntactic threshold) and routes it to a manual review queue (`PENDING_REVIEW`), allowing human analysts to intervene before the payload reaches the inference engine.
- **Forbidden Topics Enforcement**: Operator-managed forbidden topic phrases are enforced deterministically through the sanitizer, while the saved firewall prompt now also instructs the decision model to treat the configured high-level forbidden categories semantically, including indirect, paraphrased, translated, or obfuscated requests.
...
- **Global System Pause (DEFCON 1)**: A governance kill switch that prevents further automated inference in the Analyst Chat path while active. New prompts are logged and routed to the manual review queue (`PENDING_REVIEW`) instead of reaching the downstream model. The triggering ReDoS request is blocked immediately; later prompts are queued until operators resume service.
...
- **Split Prompt Roles**: System configuration keeps a **Firewall Prompt** and a **Downstream Responder Prompt** distinct. In the current runtime, the active decision model is guided by the visible firewall prompt, the active guardrails policy, optional forbidden-topics guidance, and relevant Knowledge Base policy context. The separate downstream responder prompt remains reserved for the later multi-stage responder architecture.
- **Firewall Prompt Category Guidance**: The saved baseline firewall prompt now explicitly treats the configured forbidden categories as non-allow requests even when they are indirect, semantically equivalent, paraphrased, obfuscated, or translated. It also states that storytelling and other benign formats are non-exempt when used to smuggle one of those categories, and that obvious gibberish, cipher text, or unnatural obfuscation should be treated as adversarial.
...
- **Responder Telemetry Settings**: Admin view exposes a small responder settings dialog under Analyst Chat system status. Operators can store a browser-local **LLM Base URL**, **Model ID**, and optional **Max Context Window**. The Base URL and Model ID are sent with each Analyst Chat intercept request so one browser can point clean traffic at a different downstream responder without changing shared backend defaults. When the Base URL is an OpenAI-style root such as `https://api.openai.com/v1`, the backend calls the `responses` API; API credentials remain backend-managed. **Max Context Window** is now a real browser-local submission limit used by Analyst Chat and the Prompt Playground before dispatch, while provider-reported usage still powers post-run token telemetry in Audit Logs.
...
- **Bulk Ingest Simulator**: A high-velocity ingestion utility for processing `.txt` files of prompts. Features **Base Delay + Random Jitter** (3-10s) to simulate realistic traffic patterns while respecting API rate limits. Integrated with the **Screen Wake Lock API** to maintain session persistence during long-running batch operations.
...
For the Docker demo stack on `http://localhost:3000`, the frontend uses same-origin proxying to reach the backend. Analyst Chat clean traffic still uses `/v1/intercept` even when `VITE_API_BASE_URL` is unset, and the admin-only gear under **System Status** can optionally override the downstream responder Base URL and Model ID for that browser while still keeping credentials on the backend. For OpenAI, set the Base URL to `https://api.openai.com/v1` and the Model ID to the target model; the backend will translate that into a `POST /v1/responses` call. If you also set **Max Context Window**, the browser will estimate the full forwarded request footprint, including runtime prompt scaffolding and Knowledge Base context, and block over-limit submissions before they are sent.
