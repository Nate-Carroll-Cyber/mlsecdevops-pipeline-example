...
- **Backend-Owned Responder Secrets**: Browser-side responder base URL, model, and API key overrides have been removed. Downstream LLM endpoint selection and credentials are now expected to live in backend runtime configuration or a secret manager.
- **Responder Usage Telemetry**: When the downstream provider returns `usage`, the backend now preserves prompt/completion/total token counts and the frontend can combine that with an optional admin-configured max context window to estimate context consumption in Audit Log detail views.
