# Counter-Spy.ai v2.2: Adversary-Aware Prompt Firewall & Forwarding Gateway
...
- **Authenticated Gateway Pattern**: Protected execution routes now require the backend bearer credential before work begins. `/v1/intercept`, `/v1/translate`, `/v1/instruction-monitor/reviewed-adversarial`, and `/v1/ctf/sam-spade/*` all reject unauthenticated requests; Sam Spade sessions are also scoped to the authenticated caller.
...
- **Advanced Audit Trail**: Every interaction is logged with its entropy score, detection flags, and a unique Session ID. The audit table features **multi-column sorting**, optimized column spacing, and **Full Prompt Inspection**—allowing analysts to inspect the persisted sanitized prompt, recorded obfuscation telemetry, captured model response, token usage, and split latency details in a scrollable dialog rather than exposing raw sensitive payloads back to the browser. Modal backdrops use an intentionally dark scrim and strong blur so background controls and help icons do not distract from review content. Admins can also use **Purge Session** for local demo cleanup while preserving records still pending review.
