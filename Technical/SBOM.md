...
- **Normalize - Translate Pipeline**: Prompt Playground workflow for verifying likely spelling intent before translation and then handing the translated result into the obfuscation stage. Supports browser-local heuristic normalization, optional LanguageTool correction, backend-mediated Lara translation, and structured research-log metadata for normalization backend, translation provider, detected source language, and chosen target language.
- **Backend Translation Gateway**: Playground translation routes through the local Counter-Spy.ai backend via `/v1/translate` instead of direct browser-to-provider fetches. The implementation remains manual and text-only: Lara Translate runs only when an analyst explicitly triggers the pipeline, supporting both source auto-detect -> English recovery and English -> foreign-language variant generation while keeping credentials server-side.
- **Lara Translation Modes**: The Playground now exposes two Lara-backed translation modes: **Recover to English** for analyst review and **Generate Foreign Variant** for English-to-foreign prompt generation. Provider/source/base-url tuning remains removed from the main operator flow, and translation does not run automatically on prompt edits or standard submissions.
...
- **Stable Demo Secret Path**: The Docker demo backend now reads backend-only Lara credentials from a gitignored `.env.demo.local` file so translation survives local rebuilds and container recreates without committing secrets or relying on one-off shell injection.
