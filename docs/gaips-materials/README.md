...
All jobs run in parallel after the gate passes. **The entire AI-evaluation stage is advisory** — every job here is `allow_failure: true`, so nothing it finds blocks the pipeline. Several jobs are also conditional: the live evals (`garak`, `inspect-ai`, `promptfoo`) skip when `MODEL_ENDPOINT` is unset, `giskard` scans a local stub rather than your model, and `pyrit` runs only what you wire via `PYRIT_RUN_COMMAND`. Treat this stage as evidence-gathering, not enforcement.
...
| `model-drift-detection` | Extracts normalised eval metrics (Inspect score/pass-rate, garak/pyrit/guardrail pass-rates, Giskard high-finding count, Promptfoo pass-rate) and compares them to the committed baseline `evals/eval-baseline.json`. Any metric moving beyond `DRIFT_THRESHOLD` (default ±0.10) flags model/behaviour drift on the same eval set. Report producer only (`allow_failure: true`) — the enforcing gate is `drift-gate` in the `ai-bom` stage. On first run (no baseline) it seeds `eval-baseline.seed.json` (also bundled into the end-of-pipeline `evidence-summary` artifacts for easy retrieval). **A baseline is now committed** at `evals/eval-baseline.json`, so `drift-gate` is live. It is intentionally thin — captured from a run where the live evals were skipped/stubbed — so it currently gates only the two metrics that had real values (`giskard.high_findings`, `guardrail.pass_rate`); re-seed it once the live evals are wired to cover more. |
