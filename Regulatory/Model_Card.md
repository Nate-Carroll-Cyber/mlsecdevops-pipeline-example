...
The Safeguard Effective Prompt is the reviewable policy baseline, including forbidden-category, gibberish/obfuscation guidance, and promoted few-shot examples. System Configuration previews, edits, and hashes that prompt artifact, and protected backend execution forwards the current value verbatim to the safeguard judge. `DEFAULT_SYSTEM_CONFIG` hardcodes the recommended prompt in `safeguardEffectivePromptOverride`; empty legacy values and previous app-generated baseline prompts are normalized back to that promoted default on startup, while true custom non-empty prompts are preserved as intentional drift. The current promoted recommended baseline hash and aligned current safeguard prompt hash are `590a286e60b99b0b353222b3ddaaa131db925a1f4d6222a0c3b1b3e49d203ad0`.
