...
- **Suspicious Interception**: If blocked keywords, forbidden topics, or suspicious entropy (>4.5) are detected, the request is blocked pre-inference as Suspicious.
...
- **Sam Spade CTF Intake**: The Sam Spade home tab now includes a live question field that forwards case prompts into the existing Analyst Chat pipeline under a dedicated `ctf_chat` source. This keeps the noir CTF surface aligned with the same governed sanitization, review, and audit path as the rest of Counter-Spy.ai instead of creating a separate side-channel.
- **Dedicated Sam Spade API Path**: The Sam Spade surface now uses its own backend session/message/solve API (`/v1/ctf/sam-spade/...`) with separate frontend session state and local backend persistence, instead of sharing Analyst Chat transport directly. Analyst Chat and Audit Logs now act as downstream review surfaces for Sam Spade artifacts rather than as the primary execution path.
