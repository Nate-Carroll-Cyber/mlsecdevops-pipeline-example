...
def load_baseline(path: Path) -> dict:
    ...
    # Supply-chain pin for the in-process generation model. markllm-watermark-eval
    # loads MARKLLM_MODEL_ID via transformers from_pretrained(revision=...); an empty
    # revision floats to the HF default branch, and those weights live in the HF cache
    # (not MODEL_DIR), so model-digest never hashes them. Require an IMMUTABLE commit
    # pin — a 40-hex sha, not a branch/tag (which still moves) — but only when a model
    # id is actually present (no id ⇒ no model is loaded, nothing to pin).
    mk_id = variables.get("MARKLLM_MODEL_ID")
    if mk_id:
        mk_rev = variables.get("MARKLLM_MODEL_REVISION", "")
        if not _HF_COMMIT_RE.match(mk_rev):
            _fail(
                "MARKLLM_MODEL_REVISION must be a 40-hex HuggingFace commit sha "
                f"(immutable pin) when MARKLLM_MODEL_ID is set; got {mk_rev!r}"
            )
