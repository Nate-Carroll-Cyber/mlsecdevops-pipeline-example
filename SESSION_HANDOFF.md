...
# 🔴→✅ CRITICAL FIX (2026-06-20) — `secure-software-scan` now reviews ALL accessed libraries, not just the root manifest — CODE APPLIED, awaiting one CI run to confirm

> **✅ FIX APPLIED (2026-06-20, code-only — NOT yet pushed/CI-confirmed).** `secure-software-scan` now scans the **full accessed-library surface** instead of the 3-package root `requirements.txt`:
> - **`secure_software_scan.py`** — `--requirements` is now **repeatable** (`action="append"`); pins are **merged + globally de-duplicated** across files (the core/dataquality locks overlap heavily → one search per unique package, conserving the Community quota); PEP 440 **local segments are stripped** (`torch 2.12.0+cpu` → `2.12.0`) so the purl maps to the published release the catalogue tracks; report `requirements` is now a list + `requirements_parsed` per-file counts.
> - **`.gitlab-ci.yml` `secure-software-scan`** — dropped `.resolve-reqs` (the root-manifest resolver); the job now discovers each `ci/requirements-ci*.in` GROUP and feeds one `--requirements` per group, **preferring the committed hash-pinned lock** (`requirements-ci.txt` 108, `requirements-ci-dataquality.txt` 236 — the same surface `lockfile-audit` compiles) and **falling back to the `.in`** for the markllm group (no committed lock; carries the high-risk **torch/transformers/markllm** pins). Safe because a reputation scan only parses pin strings + POSTs purls — no compile/install — so the markllm disk-exhaustion reason `lockfile-audit` skips that group does **not** apply here. Batching (5/req) + ENFORCE (`RL_FAIL_ON`) unchanged.
> - **Docs synced** — README (job table row + per-job section steps 1/3/5), `ci/SBOM.md` remediation row.
> - **Scope reckoning (user steer: "not just Python").** The reputation gate's mechanism (Spectra Assure Community purl-catalogue search) only reaches **package ecosystems** — within this static pipeline that's PyPI alone (npm/`promptfoo` is live-scans = out of scope). It CANNOT reputation-rate the other pulled classes (container images, GitHub-release binaries, model weights), which have their own controls. Per the user's decision (**map & document**, not extend/digest-pin now), added **`ci/SBOM.md` → "Supply-Chain Control Coverage by Artifact Class"**: a matrix crossing every artifact class × {pinned, CVE-scanned, malware/reputation-gated, signature/checksum-verified} + controlling jobs, plus 3 **explicit residual gaps** — (1) tool images tag-pinned not digest-pinned + not `cosign verify`'d; (2) apt/OS packages unpinned; (3) release binaries checksum- not provenance-verified. README `secure-software-scan` section cross-references it. These gaps are deliberately NOT in `secure-software-scan`'s scope (belong to image-pinning / `image-sign`).
> - **Offline-verified:** union = **304 unique pins** (108 core + 193 new dataquality + 3 markllm), torch normalized to `2.12.0`, transformers/markllm present. **Acceptance MET in principle** (evaluated reflects the full pinned stack, not 3) — **pending the actual protected-`main` CI run** to confirm `evaluated`≈300 and the gate stays green.
> - **NEXT:** push + one protected-`main` run; confirm `secure-software.json` `evaluated` is in the 100s (not 3) and `failing:[]` (or triage any real torch/transformers verdict it now surfaces).

> **✅ IMAGE SUPPLY-CHAIN HARDENING (2026-06-20, code-only — NOT pushed/CI-confirmed).** Follow-on to the coverage map's gap #1 (tool images trusted on a mutable tag). User: "do cheap, then stronger."
> - **CHEAP — digest-pinning (DONE).** All container images now pin an immutable `@sha256:` **index** digest (multi-arch) in addition to the readable `:tag`. The 8 `IMAGE_*` scanner vars got `@sha256` appended; the previously-inline base images (`python:3.11-slim` ×3 incl. `default:`, `python:3.10-slim` ×2, `security-products/secrets:4`) were centralized into new vars `IMAGE_PYTHON_311`/`IMAGE_PYTHON_310`/`IMAGE_SECRETS` (digest-pinned) and the inline refs replaced. Digests resolved locally via `docker buildx imagetools inspect <ref> --format '{{.Manifest.Digest}}'` (real digests, not guessed; daemon was down so used registry-direct imagetools). YAML parses.
> - **STRONGER — cosign provenance verify (advisory; trivy is the only verifiable image, CONFIRMED EMPIRICALLY).** New `image-provenance-verify` job (sast, `needs:[]`): installs pinned+checksum-verified cosign (mirrors `model-signing-install`) and `cosign verify`s images that carry a cosign signature. **Determined by an OCI referrers-API probe of every image (validated against trivy as a positive control):** **trivy** is the ONLY image with a discoverable signature (sigstore bundle `vnd.dev.sigstore.bundle.v0.3+json`); identity `https://github.com/aquasecurity/trivy/\.github/workflows/.+`, issuer GitHub-Actions OIDC (confirmed via Context7/Aqua docs). **anchore syft/grype, semgrep, miniconda, cyclonedx-cli, clamav, gitleaks, python, gitlab-secrets all return EMPTY referrers = unsigned upstream.** ⚠️ **CORRECTION to my earlier claim:** anchore does NOT sign its ghcr.io images either (probed both registries + both -debug/plain variants) — so there is **NO anchore→ghcr switch to make**; that "open decision" is RESOLVED as won't-help. The job logs every digest-pinned-only image EXPLICITLY (no false-assurance single green). Teeth-last via `IMAGE_VERIFY_REQUIRE` (blank=report-only, allow_failure:false, exit-code-driven like secure-software-scan).
> - **🔴→✅ FEATURE-BRANCH RUN FOUND A REAL BUG, FIXED + LOCALLY PROVEN (2026-06-20).** First feature-branch pipeline (branch pushed to gitlab): `image-provenance-verify` "passed" (report-only) but **trivy cosign verify FAILED** (`image-provenance.json` verify_failures:1) — so it was verifying ZERO images. Two defects: (1) my script swallowed cosign stderr (`>/dev/null 2>&1`) → the log hid WHY; (2) **cosign 2.4.1 cannot read trivy's signature** — it's stored as an OCI-referrers `sigstore.bundle.v0.3` artifact with NO `.sig` tag (confirmed by pulling the bundle from the Docker Hub referrers API + decoding the Fulcio cert: SAN `…/aquasecurity/trivy/.github/workflows/reusable-release.yaml@refs/tags/v0.71.1`, issuer GH-Actions OIDC). cosign 2.4.1 `verify` → "no signatures found"; `--registry-referrers-mode` doesn't exist in 2.4.1 (`--experimental-oci11` also failed). **Per trivy docs (web) cosign v3+ is required; verified LOCALLY** (downloaded cosign v3.1.1 darwin — image verify needs NO docker daemon, only registry/Rekor/Fulcio net): plain `cosign verify aquasec/trivy@<digest> --certificate-identity-regexp 'https://github\.com/aquasecurity/trivy/\.github/workflows/.+' --certificate-oidc-issuer https://token.actions.githubusercontent.com` → **EXIT 0, 2 signatures**. **FIX:** new var `COSIGN_VERIFY_VERSION="v3.1.1"` (separate from signing jobs' `COSIGN_VERSION=v2.4.1` — a 2→3 bump there is unrelated breaking-change risk); job installs v3 (checksum-verified, v3.1.1 checksums file format confirmed); `verify_one` now strips `:tag` → `repo@digest` and **no longer swallows stderr**. Re-push to confirm green on CI, then `IMAGE_VERIFY_REQUIRE=true`.
> - **Docs:** `ci/SBOM.md` — Container Images note (all digest-pinned + re-resolve cmd), coverage matrix row updated, residual gap #1 rewritten (integrity ✅ / provenance ⚠️ partial), 2 remediation rows.
> - **NEXT (image work):** (1) CI-confirm `image-provenance-verify` runs green (trivy verifies), then set `IMAGE_VERIFY_REQUIRE=true` to gate; (2) digests captured 2026-06-20 — re-resolve when bumping any image tag; (3) image provenance is now "done as far as upstreams allow" — only trivy is signable, so no more images to add unless a publisher starts signing.

> **Status (prior, session-10):** the `secure-software-scan` ReversingLabs Spectra Assure Community gate is **LIVE and GREEN on protected `main`** (merge `d36b45f`; the protected `RL_TOKEN` injected — `length=453`, `TOKEN OK`, mode=ENFORCE malware,tampering, 3 evaluated, `failing:[]`, gate PASSED). The wiring works. **But its coverage was wrong, and this was a CRITICAL fix.**
>
> **The gap:** the job scans only the **3 root `requirements.txt` deps** (pandas / requests / jinja2) — the *exact same* narrow surface as `pip-audit`. It does **NOT** review the libraries the pipeline actually installs and executes: the full ML/data stack pinned in the `ci/requirements-ci*.in` GROUP lockfiles — **torch, transformers, markllm**, presidio, evidently, ydata-profiling, great-expectations, modelscan, sigstore/model-signing, etc. (core lock = 108 pkgs, dataquality = 236, plus the markllm group).
>
> **Why CRITICAL (not nice-to-have):** the malware/tampering reputation gate is **blind to the real supply chain**. The known RCE-class vulns already surfaced in this repo live in exactly the libs it can't see — `markllm-deps-audit` (#34) found torch 2.12.0 (CVE-2025-3000) and transformers 4.57.6 (X-CLIP deserialization RCE / Trainer `torch.load` RCE). A clean green on 3 trivial root deps gives **false assurance** the supply chain was vetted for malware/tampering when the high-risk libraries were never checked.
>
> **Required fix:** point `secure-software-scan` at the **compiled group lockfiles** (the same surface `lockfile-audit` already covers — every library actually accessed/installed), not the root `requirements.txt`. Keep the Community 5-packages/request batching (already implemented) and ENFORCE mode. Consider per-group invocation mirroring `lockfile-audit` so torch/transformers/markllm get a reputation+tampering verdict, not just a CVE audit. **Acceptance:** the scan's `scanned`/`evaluated` count reflects the full pinned stack (100s of pkgs), not 3.
...
>
> **#2 prereqs verified via API:** default branch = `main` (protected). `MODEL_SIGNING_IDENTITY` + `SIGSTORE_OIDC_ISSUER` exist and are `protected=True` → that's WHY `signature-verification` (#19) + keyless identity binding deferred on the feature branch; on `main` they run for REAL (the payoff of #2). `GITLAB_PUSH_TOKEN` MISSING → `data-drift-baseline-commit` SKIPs (no baseline push) — fine, drift is deliberately not chased (vacuous on the single-class gandalf set). `DT_API_*`/`MODEL_ENDPOINT`/`VAULT_ADDR` missing → inert by design.
>
> **✅ #2 DONE — merged to `main` (MR !1, merge commit `58ad3b1`); protected pipeline `2615345388` ALL 49 JOBS GREEN.** The signing/identity legs open since session 5 finally ran for real:
> - **`signature-verification` (#19) genuinely VERIFIED** (not the feature-branch DEFER): identity `…/.gitlab-ci.yml@refs/heads/main`, issuer `https://gitlab.com`, "Verification succeeded", cert SAN matches the pinned identity, **Rekor `logIndex=1874436658`**, "1 signature(s) verified against the pinned identity". It now PRINTS the SAN/issuer/Rekor index/digest → **closes the long-standing #19 explainability gap** (proves WHAT it verified against, not just THAT it verified).
> - **`model-sign`, `ai-bom-sign`, `tamper-verification`** all GREEN **hard gates** (`allow_failure:false`) on the protected ref — keyless identity binding validated end-to-end.
> - **`lockfile-audit`: 0 `[Errno 28]` lines** on the protected run — the disk fix is confirmed clean; core + dataquality locks compiled, `pip-audit --no-deps` did not install.
> - **`data-drift-baseline-commit` correctly SKIPPED** ("GITLAB_PUSH_TOKEN not set — auto-commit disabled") → no surprise commit to `main`, no runaway pipeline. Drift stays inert by design.
>
