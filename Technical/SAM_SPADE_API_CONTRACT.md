...
- `POST /v1/ctf/review-artifacts` — body `{ "artifact": <SamSpadeReviewArtifact> }`. The CTF frontend posts each turn's review artifact here (best-effort). The gateway persists it to a **SQLite store** (`backend/src/ctf/reviewArtifactStore.ts` — WAL, count-capped at `CTF_REVIEW_ARTIFACTS_MAX`, path via `CTF_REVIEW_ARTIFACTS_STORE_PATH`), emits a `ctf.review_artifact` metric, and logs `ctf_review_artifact`. Returns `202 { "ok": true }`. (Auth: `requireBackendAuth`.)
- `GET /v1/ctf/review-artifacts?sinceTimestamp=<ISO>&limit=<n>` — returns `{ "artifacts": [ ... ] }` (the latest `limit`, returned oldest→newest; optionally filtered to artifacts strictly after `sinceTimestamp`).

The main Counter-Spy frontend's Sam Spade tab embeds the CTF app via an `<iframe>` (`VITE_CTF_FRONTEND_URL`; a placeholder when unset) and **polls** `GET /v1/ctf/review-artifacts` (only artifacts created after the poll starts) — for each new one it mirrors the turn into Analyst Chat and writes an `audit_logs` doc (source `ctf_chat`), exactly the shape the in-app path produced. The store survives a gateway restart but is bounded (oldest pruned) — a demo bridge, not a durable analytics archive. The in-app CTF UI fallback that previously drove the CTF routes directly was removed once the container path became the deployed one.
