...
- **Prompt Role Separation**: System configuration distinguishes the generated Safeguard Effective Prompt from the Downstream Responder Prompt. The safeguard judge receives a generated Safeguard Effective Prompt built from the internal firewall baseline, guardrails policy, forbidden phrases, Knowledge Base excerpts, backend-owned JSON verdict contract, and neutral evidence contract. The downstream responder prompt is sent only after clean traffic clears the gateway and responder routing is enabled.
...
- **Structured Decision Contract**: The firewall configuration now relies on a structured decision contract centered on `ALLOW_AND_FORWARD`, `BLOCK`, `QUEUE_FOR_REVIEW`, and `FAIL_SECURE`, and those decisions are mapped back into Counter-Spy.ai audit severity/review state before legacy text heuristics are consulted.
