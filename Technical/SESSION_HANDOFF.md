...
### Phase 3 step 2 — rewire `App.tsx` to `/v1/audit-logs` (next)
Replace the browser→Firestore audit path with the `backendApi.ts` audit clients:
- **Writes:** `addDoc(collection(db, 'audit_logs'), {...})` (2 sites in `handleSendMessage` — the context-window-block branch ~3500s and the main branch ~3660s — plus `runBulkIngest`) → `await appendAuditLog(record)`. Drop `userId`/`id`/`timestamp` from the payload (the backend stamps them). The `localReviewMode` branch still pushes onto local `auditLogs` state (no backend) — keep that fallback for now (auth deferred), or have `localReviewMode` also POST since the backend trusts the caller-id header — TBD.
- **Reads:** the `onSnapshot(query(collection(db, 'audit_logs'), orderBy('timestamp','desc'), limit(...)))` subscription that populates `auditLogs` → poll `listAuditLogs({ limit })` on an interval (e.g. every few seconds while the Audit/Metrics tabs are active) and `setAuditLogs(rows.map(r => parseAuditLog({ ...r.record })))` — `r.record` already has `id`/`userId`/`timestamp` merged in. Existing `parseAuditLog`/`AuditLogSchema` (App.tsx ~348/1828) does the strict validation.
- **Mutations:** `handleReviewLog`/`handleRetryAuditLog`/`handlePromoteToKB` etc. that do `updateDoc(doc(db,'audit_logs',id), patch)` → `await patchAuditLog(id, patch)`. `handleClearAuditLogs`/`deleteAllAuditLogs` (`writeBatch`/`deleteDoc`) → `await clearAuditLogs()`. CSV/golden-set export reads from local `auditLogs` state — unchanged.
- Gotchas: the audit table is the **shared** SOC trail (not per-user) — `listAuditLogs()` with no `userId` filter returns everything; RBAC on review/promote/clear is deferred (auth deferred). The Firestore `serverTimestamp()` becomes the Postgres `created_at` (ISO string in `record.timestamp`). The Firestore rules allowlist (rejecting backend-owned fields on client creates) has no equivalent yet — Phase 3(4) re-expresses it.

### Phase 3 step 3 — rewire `ThreatDashboard.tsx` + move `anomalyDetector`/`metrics` server-side
