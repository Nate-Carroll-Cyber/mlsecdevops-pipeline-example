...
- **Safeguard Effective Prompt Review**: System Configuration displays and hashes the recommended and current effective safeguard prompt text so intentional prompt updates can be reviewed as configuration artifacts. `DEFAULT_SYSTEM_CONFIG` hardcodes the recommended prompt in `safeguardEffectivePromptOverride`; empty legacy config values and app-generated legacy baseline variants are normalized back to that promoted default on startup, so first-open deployments align without requiring an operator reset. Genuinely custom non-empty prompts are preserved and still show drift. The current promoted recommended baseline hash and aligned current safeguard prompt hash are `590a286e60b99b0b353222b3ddaaa131db925a1f4d6222a0c3b1b3e49d203ad0`. The protected backend gateway accepts the allowlisted `safeguardEffectivePrompt` field and forwards it verbatim to the safeguard judge; provider safeguard calls fail closed if that field is absent instead of using a backend-authored fallback prompt.
- **Safeguard Category Guidance**: The recommended Safeguard Effective Prompt includes the baseline forbidden-category, forbidden-phrase, and gibberish guidance operators review in System Configuration. Clear forbidden-category requests, such as medication or dosage recommendations, are classified as `ADVERSARIAL` blocks; `SUSPICIOUS` is reserved for ambiguous or review-needed evidence.
