...
| `modelaudit-scan` | Runs Promptfoo ModelAudit (`promptfoo scan-model`) after `modelscan` across `models/`, including GGUF/GGML, safetensors, ONNX, manifests, archives, and other static model artifacts. The job installs broad ModelAudit extras for framework, cloud, MLflow, and NumPy compatibility paths, falling back to the broad non-NumPy-compat set if dependency solving requires it. It writes `modelaudit.json` plus a normalized `modelaudit-summary.json`; `artifact-signing-gate` blocks on operational scan failure or CRITICAL findings. |
