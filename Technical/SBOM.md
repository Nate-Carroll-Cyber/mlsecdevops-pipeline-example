...
- **Forbidden Phrases Governance**: Configurable normalized phrase-family matching for forbidden phrases. The visible recommended Firewall Prompt now includes the baseline forbidden-category and gibberish/obfuscation guidance used by the safeguard judge, while active phrase configuration can still supplement runtime context.
...
- **Prompt Role Separation**: System configuration distinguishes the visible Firewall Prompt from the Downstream Responder Prompt. The safeguard judge receives a generated Safeguard Effective Prompt built from the visible Firewall Prompt, guardrails policy, forbidden phrases, Knowledge Base excerpts, backend-owned JSON verdict contract, and neutral evidence contract. The downstream responder prompt is sent only after clean traffic clears the gateway and responder routing is enabled.
...
- **Visible Safeguard Category Guidance**: The recommended Firewall Prompt is the reviewable baseline for category and gibberish guidance. The backend/frontend runtime can still provide the safeguard judge with active forbidden phrase context from configuration without replacing that visible baseline.
