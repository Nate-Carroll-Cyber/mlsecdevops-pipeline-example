...
- **Anti-ReDoS Circuit Breaker**: High-resolution timing (`performance.now()`) monitors the sanitization pipeline. If a sanitization pass completes above 100ms, the triggering request is blocked before inference, logged as `Adversarial` with `ReDoS_ATTEMPT_DETECTED`, and automatic Global System Pause is activated for subsequent traffic.
