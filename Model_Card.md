...
The Safeguard Effective Prompt is the reviewable policy baseline, including an authoritative **Forbidden Phrases and Questions** section, gibberish/obfuscation guidance, business-benefit denials, and promoted few-shot examples. System Configuration previews, edits, and hashes that exact effective prompt. `DEFAULT_SYSTEM_CONFIG` hardcodes the recommended prompt in `safeguardEffectivePromptOverride`; empty legacy values and previous app-generated baseline prompts are normalized back to that promoted default on startup, while true custom non-empty prompts are preserved as intentional drift. The current promoted recommended baseline hash and aligned current safeguard prompt hash are `49bcc951d8af376818acf0a2edef5411edd9bf4d06a0848a5037d19f13917881`.
