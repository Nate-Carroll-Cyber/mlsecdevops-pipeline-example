...
**Step by step, in plain English**
1. Runs on the default Python image, `needs: []` (early, independent of the rest of `sast`). Installs the pinned `cosign` binary and verifies it against its published checksum (`sha256sum --check --strict`), identical to `model-signing-install`.
...
5. Applies the **enforcement switch** `IMAGE_VERIFY_REQUIRE` (teeth-last, mirrors `RL_FAIL_ON`): blank → report-only (`exit 0` even on a verify failure); `true` → the job `exit 1`s if a *signed* image fails to verify. Unsigned/digest-only images never gate. Because the switch lives in the script's exit code, `allow_failure:false`.
