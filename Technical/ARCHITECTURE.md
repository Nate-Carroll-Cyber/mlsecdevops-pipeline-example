...
2.  **The Sword (Backend-Mediated Inference):** The backend `/v1/intercept` route runs deterministic prechecks, compares clean candidates against the pgvector-backed instruction similarity monitor when enabled, and calls an OpenAI-compatible safeguard judge before any downstream responder receives a prompt. Analyst Chat safeguard configuration remains separate from responder configuration, allowing Counter-Spy.ai to sit between different frontier model providers. Safeguard and responder credentials and model endpoints are managed on the backend; browser callers cannot override those provider runtime values on protected execution paths.
    *   **Current prompt-contract note:** The firewall stage is guided by the current System Configuration **Safeguard Effective Prompt**, forwarded verbatim by `/v1/intercept` as the safeguard system prompt. The default System Configuration hardcodes the recommended prompt in `safeguardEffectivePromptOverride`; empty legacy values and prior app-generated baseline prompts are normalized back to that promoted default on startup, while true custom prompt overrides are preserved as intentional drift. Clear forbidden-category requests, including medication or dosage advice, must return `ADVERSARIAL` and block; `SUSPICIOUS` is reserved for ambiguous or review-needed evidence. System Configuration still displays and hashes the recommended/current safeguard prompt artifacts for review and drift detection; the current promoted recommended baseline hash and aligned current safeguard prompt hash are `49bcc951d8af376818acf0a2edef5411edd9bf4d06a0848a5037d19f13917881`. The safeguard judge receives a candidate prompt after deterministic normalization/redaction plus neutral preprocessing evidence, not the local sanitizer's final verdict or reasoning. Only prompts the safeguard judge returns as `CLEAN` are forwarded to the responder model when responder routing is enabled; otherwise clean traffic returns local responder passthrough. Legacy decision-shaped safeguard payloads, malformed JSON, and non-JSON outputs are treated as `SUSPICIOUS` and queued for review rather than normalized into an allow path. Safeguard upstream calls are bounded by `SAFEGUARDS_TIMEOUT_MS` (default 30s); timeout or provider failure returns structured `SHIELD_ERROR` fail-secure telemetry instead of an unstructured transport error.
    *   **Current forbidden-category note:** Configured forbidden phrases are enforced locally and included in the Safeguard Effective Prompt's authoritative **Forbidden Phrases and Questions** section. The safeguard judge treats entries as semantic phrase families, not exact-string examples; close synonyms, paraphrases, inflections, percentages, role framing, approval/confirmation language, and indirect requests block as `ADVERSARIAL`. Uncertain matches queue as `SUSPICIOUS`.
...
### 2.5 Structural Jailbreak Signals
...
*   **Logic:** The cardinal rule ("nothing reaches the responder without passing the sanitizer") has an outbound counterpart: every responder LLM string is re-run through `sanitizeOutput` (shared: `packages/backend-shared/src/security/sanitizer.ts`) before it leaves the gateway. `sanitizeOutput` reuses the inbound redaction passes (`SENSITIVE_PATTERNS`, credit-card, redacted-placeholder echo) plus the blocked-keyword check; it does **not** apply entropy/syntactic scoring because model output is not an adversarial vector for those.
...
### 4.3 Rate Limiting
*   **Logic:** A dependency-free fixed-window limiter (`packages/backend-shared/src/middleware/rateLimit.ts`) runs immediately after the JSON body parser, keyed by the caller's bearer token (so a leaked credential cannot be used to run up the safeguard/responder bill) and falling back to the client IP. `/healthz` is exempt.
...
| `metadata` | `object` | Strict allowlist only: `localReviewMode`, `source`, `providerLlmRoutingEnabled`, `responderLlmRoutingEnabled`, current `safeguardEffectivePrompt`, optional browser-memory `safeguardApiKey`, and optional instruction-monitor embedding fields. Browser callers cannot choose backend provider endpoints, model base URLs, or responder system prompts. |
...
**Responder passthrough modes.** There are two distinct ways the gateway can avoid making a downstream responder call after a CLEAN safeguard verdict, and the two are not interchangeable:

| Mode | Trigger | Surface |
| :--- | :--- | :--- |
| **Operator passthrough** (preferred) | Browser sends `metadata.responderLlmRoutingEnabled: false` (the Analyst Runtime Settings → Responder LLM Routing toggle). | Backend short-circuits with the `DISABLED_LOCAL_ONLY` response: `responder.status: 'DISABLED_LOCAL_ONLY'`, `responder.modelId: 'local-responder-passthrough'`, `responder.response: LOCAL_RESPONDER_PASSTHROUGH_RESPONSE_TEXT`. Audits/telemetry record this as the intentional analyst choice. |
| **Env-unconfigured passthrough** (safety net) | Any of `LLM_API_BASE_URL` / `LLM_API_KEY` / `LLM_MODEL_ID` (or their `RESPONDER_*` aliases) is missing or empty. Browser thinks the responder is enabled but the backend has nowhere to send the request. | Backend returns `'Counter-Spy.ai backend accepted this clean prompt. Configure responder provider, API key, base URL, and model ID to enable live downstream inference.'` with `latencyMs: 0` and the configured `provider`/`modelId`. Surfaced so operators notice and either wire up a responder or toggle the routing switch off. |

The demo stack (`docker-compose.demo.yml` + `.env.demo.local`) intentionally leaves `LLM_API_*` unset and ships with the env-unconfigured safety net in play — set `LLM_API_BASE_URL` + `LLM_API_KEY` + `LLM_MODEL_ID` (or the `RESPONDER_*` triple) when wiring an actual downstream responder. The Analyst Runtime Settings selector is the operator-facing control; the env is a backend-side gate so a misconfigured route can never reach an upstream provider.

**Safeguard effective prompt — cross-surface fallback.** The Analyst Chat console always forwards its current `safeguardEffectivePrompt` in `/v1/intercept`'s `metadata` (operators tune it in System Configuration), and `resolveSafeguardJudgeInstructions` throws when that field is missing — the analyst-side strict contract is preserved by `securityRoutes.test.ts`. The Sam Spade CTF iframe (`ctf-frontend/`) is a separate bundle that doesn't share localStorage or Firestore with the analyst console and currently has no UI to author its own safeguard prompt, so the `/v1/ctf/sam-spade/message` handler falls back to a backend-owned constant — `DEFAULT_SAFEGUARD_EFFECTIVE_PROMPT` in [`packages/backend-shared/src/security/safeguardDefaults.ts`](../packages/backend-shared/src/security/safeguardDefaults.ts) — when metadata is missing or empty. The default carries the load-bearing parts of the judge contract (`{verdict, analystReasoning}` JSON-only output, classification rules, neutral-evidence contract) but omits the operator-tunable bits (firewall prompt, guardrails policy, forbidden topics, few-shot examples). Phase 3 step 4 moves governance config to Postgres; once that lands, both surfaces will read the active operator-set prompt from the governance store and this constant can shrink to only the structural judge-output rules.
...
*   **Review Workload Rollup:** Metrics does not rewrite audit severity. For operational counts only, unreviewed `Suspicious` results roll up into `Review`, so the Alert Severity trend and HITL Queue `Pending Review` count reflect the analyst workload created by borderline detections.
*   **Alerting Thresholds:** 
...
The backend is instrumented with the OpenTelemetry SDK (`packages/backend-shared/src/telemetry.ts`, imported first by both `services/gateway/src/server.ts` and `services/sam-spade/src/server.ts`).
...
*   **No-op by default:** The SDK only starts when an OTLP endpoint/exporter is configured. With nothing set — or with `OTEL_SDK_DISABLED=true` — the OpenTelemetry API resolves to no-op providers and the only sink is the structured stdout JSON, so `npm run gateway:dev` and the test suite stay quiet.
...
The gateway and the noir CTF surface are now **separate npm workspaces with separate Dockerfiles**, sharing source only through `@counter-spy/backend-shared`.

*   **Gateway** (`services/gateway/`, image `services/gateway/Dockerfile`) — serves `/v1/intercept`, `/v1/analyze*`, `/v1/translate`, `/v1/instruction-monitor/*`, `/v1/ctf/review-artifacts`, and the server-rendered analyst console. The `/v1/ctf/sam-spade/*` paths are **not** served by the gateway; the CTF frontend and main console call the Sam Spade service directly over the network.
*   **Sam Spade service** (`services/sam-spade/`, image `services/sam-spade/Dockerfile`) — boots only `/healthz` and `/v1/ctf/sam-spade/*` on `SAM_SPADE_SERVICE_PORT` (default `18120`). It owns the SQLite session store volume and makes its own safeguard/responder calls; it runs the same `requireBackendAuth` (shared `INTERCEPT_BEARER_TOKEN`) and rate limiter from `packages/backend-shared/`.
*   **CTF frontend container** — the noir UI (`ctf-frontend/`, image `Dockerfile.ctf-frontend`) runs on its own. It talks to the gateway, plays the game, and `POST`s each turn's review artifact to `POST /v1/ctf/review-artifacts` — a **SQLite-backed** store on the gateway (`services/gateway/src/ctf/reviewArtifactStore.ts`, WAL, count-capped at `CTF_REVIEW_ARTIFACTS_MAX`; `GET /v1/ctf/review-artifacts?sinceTimestamp=&limit=` reads it back, latest-first). The main Counter-Spy frontend's Sam Spade tab embeds the CTF app via an `<iframe>` (`VITE_CTF_FRONTEND_URL`; a placeholder when unset) and polls that feed (only artifacts created after the poll starts) to mirror CTF turns into Analyst Chat + `audit_logs` (source `ctf_chat`) — so Audit/Metrics keep surfacing CTF activity even though the main frontend no longer drives the CTF API itself. (The in-app CTF UI fallback and its session state/handlers were removed once this is the deployed path.)
*   **Demo topology** (`docker-compose.demo.yml`): the SSR analyst console served by `counter-spy-backend (gateway)` embeds `counter-spy-ctf-frontend`, which talks **directly** to `counter-spy-sam-spade-service` on `:18120` for `/v1/ctf/sam-spade/*` and to the gateway for `/v1/ctf/review-artifacts`. The gateway no longer proxies CTF traffic — it 404s `/v1/ctf/sam-spade/*` — but still persists CTF review artifacts so the CTF surface keeps working even if the gateway briefly restarts. Runs alongside `postgres`, `otel-collector`, and `jaeger`. `docker-compose.sam-spade.yml` runs just the CTF service (behind the `sam-spade` profile).
*   The HTTP contract (see `Technical/SAM_SPADE_API_CONTRACT.md`) is unchanged across the split. The review-artifact store survives a gateway restart but is still bounded (oldest pruned) — a demo bridge, not a durable analytics archive.
