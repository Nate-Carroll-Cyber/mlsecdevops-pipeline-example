...
**Version:** v2.0 (Promotion to Beta)  
...
- **Forbidden Topics Governance**: Configurable normalized phrase-family matching for blocked topics, combined with explicit forbidden-category guidance inside the saved firewall prompt so the active decision model can treat indirect, paraphrased, translated, or obfuscated forms as non-allow categories.
...
- **Interactive Governance Controls**: Real-time administrative controls synchronized via Firestore. Includes **Human-in-the-Loop (HITL) Mode** for routing borderline traffic to a manual review queue, and **Global System Pause (DEFCON 1)**, a governance kill switch that routes new prompts into `PENDING_REVIEW` instead of downstream inference while the pause is active.
...
- **Policy Violation Signal**: Blocked keywords, forbidden topics, and regex-rule hits now emit a shared `POLICY_VIOLATION` signal so the logs, exports, and response messaging distinguish explicit policy breaches from generic suspicious traffic.
...
- **Bulk Ingest Simulator**: High-velocity ingestion utility for synthetic traffic generation and adversarial testing. Features **Base Delay + Random Jitter** (3-10s) and **Screen Wake Lock API** integration for session persistence. Supports **DPO Pre-labeling** (Batch ID, Expected Verdict) for training data acquisition and now accepts one-line prompts, explicit `===PROMPT===` / `===END===` blocks, blank-line-separated multi-line prompts, and common numbered-entry layouts.
