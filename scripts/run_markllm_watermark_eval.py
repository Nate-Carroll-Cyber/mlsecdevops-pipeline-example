...
def main() -> None:
    ...
    # max_new_tokens caps NEW tokens; min_length is a floor on the TOTAL sequence
    # length (prompt + generated). transformers derives max_length = prompt_len +
    # max_new_tokens, so if min_length > max_length the floor is unsatisfiable:
    # transformers warns ("Unfeasible length constraints") and silently drops it.
    # Keep the max_new_tokens default >= the min_length default so the floor is
    # always feasible for any prompt length and the recorded run stays warning-free.
    parser.add_argument("--max-new-tokens", type=int, default=int(os.environ.get("MARKLLM_MAX_NEW_TOKENS", "256")))
