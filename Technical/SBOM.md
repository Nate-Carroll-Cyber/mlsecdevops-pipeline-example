...
- **Sam Spade Response Model Boundary**: Clean Sam Spade turns are still satisfied by deterministic in-service noir reply logic after guardrail approval; they do not yet use the live downstream LLM responder path that Analyst Chat can exercise.
...
- **Bulk Ingest Simulator**: High-velocity ingestion utility for synthetic traffic generation and adversarial testing. Features **Base Delay + Random Jitter** (3-10s) and **Screen Wake Lock API** integration for session persistence. Supports **DPO Pre-labeling** (Batch ID, Expected Verdict), displays the selected `.txt` filename under the app-rendered file chooser after upload, stops active replay when Global System Pause is initiated, and accepts one-line prompts, explicit `===PROMPT===` / `===END===` blocks, blank-line-separated multi-line prompts, and common numbered-entry layouts.
